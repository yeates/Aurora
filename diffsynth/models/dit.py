...
from diffsynth.models.state_dict_utils import hash_state_dict_keys
try:
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, t_mod, freqs, modulate_index=None):
        if modulate_index is not None:
            if t_mod.shape[0] % 2 != 0:
                raise ValueError(f"Expected doubled timestep embeddings for modulate_index, got {tuple(t_mod.shape)}")
            batch_size = t_mod.shape[0] // 2
            if batch_size != x.shape[0]:
                raise ValueError(f"t_mod batch {batch_size} does not match hidden batch {x.shape[0]}")
            seq_len = x.shape[1]
            modulation = self.modulation.to(dtype=t_mod.dtype, device=t_mod.device) + t_mod
            modulation_target = modulation[:batch_size].unsqueeze(2).expand(batch_size, 6, seq_len, self.dim)
            modulation_ref = modulation[batch_size:].unsqueeze(2).expand(batch_size, 6, seq_len, self.dim)
            token_selector = modulate_index.to(device=x.device, dtype=torch.bool).unsqueeze(1).unsqueeze(-1)
            modulation = torch.where(token_selector, modulation_ref, modulation_target)
...
class ConditionalEmbedder(nn.Module):
    def __init__(self, in_dim, dim, patch_size, zero_init=False, max_items=None):
        super().__init__()
        self.patch_embedding = nn.Conv3d(
            in_dim, dim, kernel_size=patch_size, stride=patch_size)
        if zero_init:
        ...
        self.max_items = 0 if max_items is None else int(max_items)
        if self.max_items > 0:
            self.index_embedding = nn.Embedding(self.max_items, dim)
            nn.init.normal_(self.index_embedding.weight, std=0.02)
        else:
    ...
    def add_item_embeddings(self, x):
        if self.index_embedding is None:
            return x
        if x.dim() != 5:
            raise ValueError(f"Expected 5D ref tensor, got shape {tuple(x.shape)}")
        num_items = x.shape[2]
        if num_items > self.max_items:
            raise ValueError(f"num_items={num_items} exceeds max_items={self.max_items}")
        item_ids = torch.arange(num_items, device=x.device)
        item_embeds = self.index_embedding(item_ids).to(dtype=x.dtype)
        item_embeds = rearrange(item_embeds, "f c -> 1 c f 1 1")
        return x + item_embeds

    def add_single_item_embedding(self, x, item_id):
        if self.index_embedding is None:
            return x
        if x.dim() != 5:
            raise ValueError(f"Expected 5D ref tensor, got shape {tuple(x.shape)}")
        if item_id >= self.max_items:
            raise ValueError(f"item_id={item_id} exceeds max_items={self.max_items}")
        item_embed = self.index_embedding.weight[item_id].to(dtype=x.dtype, device=x.device)
        item_embed = item_embed.view(1, -1, 1, 1, 1)
        return x + item_embed
