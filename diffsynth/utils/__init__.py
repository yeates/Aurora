...
def _requires_negative_cfg_branch(inputs_shared: dict) -> bool:
    for key in ("cfg_scale", "image_cfg_scale"):
        value = inputs_shared.get(key, 1)
...
class PipelineUnitRunner:
    ...
    def __call__(self, unit: PipelineUnit, pipe: BasePipeline, inputs_shared: dict, inputs_posi: dict, inputs_nega: dict) -> tuple[dict, dict]:
        if unit.take_over:
            ...
            if _requires_negative_cfg_branch(inputs_shared):
...
from diffsynth.utils.tar_shard import ShardIndex, TarSlicePath, get_active_shard_index, set_active_shard_index
